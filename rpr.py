    ...
    if uploaded_file is not None:
        with st.spinner("Processing Analytics"):
             for i in range(len(avg_curve1)):
                 ...
                 if needlesize == 14:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (5.12 * 10**-10)
                 if needlesize == 15:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (3.23 * 10**-10)
                 if needlesize == 16:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (2.13 * 10**-10)
                 if needlesize == 17:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (1.52 * 10**-10)  
                 if needlesize == 18:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (7.36 * 10**-11)
                 if needlesize == 19:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (4.04 * 10**-11)
                 if needlesize == 20:
                        Q = ((((0.6 * curve)/needlesize) / time)) * (2.74 * 10**-11)
                 else:
        ...
        shears = px.scatter(rrf, x='Shear Rate', y='Relative Resistance to Flow', color_discrete_sequence=['orange'], trendline="rolling", trendline_options=dict(window=10),hover_data=["mmHg range", "Blood Sample", "Water Control"])
